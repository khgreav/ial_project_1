...
*/
    if(q != NULL) { //queue exists, initialize
        for(int i = 0; i < QUEUE_SIZE; i++) {
            q->arr[i] = '*';
        }
        q->f_index = 0;
        q->b_index = 0;
    } else { //q has a NULL, call queueError
        queueError(QERR_INIT);
    }
}
...
*/
    if(queueEmpty(q) == 0) { //queue not empty, get front value
        *c = q->arr[q->f_index];
    } else { //queue empty, call queueError
...
*/
    if(queueEmpty(q) == 0) { //queue not empty, get next index
...
*/
    if(queueEmpty(q) == 0) { //queue not empty, remove first element
        queueFront(q, c);
        queueRemove(q);
    } else { //queue empty, call queueError
        queueError(QERR_GET);
...
*/
    if(queueFull(q) == 0){ //queue not empty, add element at the end of the queue
        q->arr[q->b_index] = c;
